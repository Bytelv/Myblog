document.addEventListener("DOMContentLoaded", function() {
  ...
  // 根据消息数据生成HTML元素
  function generateChannelMessageItem(id, text, time, views) {
  ...
  function showChannelMessages(jsonData, domId) {
    ...
    for (var key in channelMessageData) {
      if (channelMessageData.hasOwnProperty(key)) {
        var messageData = channelMessageData[key];
        var formattedTime = qexoFormatTime("YYYY-mm-dd HH:MM:SS", Number(messageData['time']));
        html += generateChannelMessageItem(key, messageData['text'], formattedTime, messageData['views'] || 0);
      }
    }
